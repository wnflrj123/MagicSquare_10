...
| 계층 | 이름 예시 | 수명 | 머지 대상 | 직접 커밋 허용? |
| --- | --- | --- | --- | --- |
| 통합 | `main` | 영구 | (배포 가능 상태만) | ❌ |
| 개발 | `develop` | 영구 | `main` (리뷰 후) | ⚠️ 문서 작업만 예외 |
| 작업 | `feature/<invariant-id>-<짧은-이름>` | 단기 (수 시간 ~ 수 일) | `develop` (PR) | ✅ |

> 본 보고서가 들어 있는 현재 작업(`Report/`, `Prompt/` 추가)은 **문서 작업**이므로 `develop` 직접 커밋의 예외 케이스에 해당한다.
